...
// Enable LEDs.
//
// 1. The LEDs are attached to 'GPIO PORT D' PD12 - PD15. (Ref: #3 Tab 7).
// 2. GPIO PORT D is attached to the 'AHB1' 168 MHz bus. (Ref: #2 Fig 5).
// 3. AHB1 PORT D Clock is enabled via the 'RCC AHB1 peripheral clock register (RCC_AHB1ENR)'. (Ref: '#1 Sec 6.3.10).


// The LEDs are attached to 'GPIO PORT D' PD12 - PD15. (Ref: #3 Tab 7).

// RCC AHB1 peripheral clock register (RCC_AHB1ENR)'. (Ref: '#1 Sec 6.3.10).
...
// GPIO port mode register (GPIOx_MODER). (Ref: '#1 Sec 8.4.1).
// What bits in the 'mode register' need to be set to output to switch the LEDs on?
...
// GPIO port output data register (GPIOx_ODR). (Ref: '#1 Sec 8.4.6).
